...
const URL = process.env.CHECKERS_URL || `ws://localhost:${process.env.CHECKERS_PORT||3011}`;
...
(async()=>{
  ...
  console.log('[checkers] quickChat OK ->', got.username, got.text);
  a.close(); b.close();
  process.exit(0);
})().catch(e=>{ console.error('[checkers] quickChat FAIL', e.message); process.exit(1); });
