...
// ---------- Deck visual (kept minimal) ----------
function DeckStack({ backTex, x=0.50, y=0.12 }){
...
// ---------- Shader that shows BACK when the plane faces camera, FRONT when turned over ----------
function CardFace({ frontTex, backTex }){
  ...
  const fragment = /* glsl */`
    ...
    void main(){
      // BACK when facing camera, FRONT when flipped
...
// ---------- Single-plane flip with synchronized travel ----------
function FlipThenTravelCard({
  ...
  finalRotZ=0,
  dealDelay=0,
  totalMs=1000,       // total animation time from lift to touchdown
  zLift=0.035,
  edge='right',       // 'right' edge lifts (CCW look), 'left' for CW
  arc=0.02            // arc height during travel
}){
    ...
    if (!started.current){
      if (clock.current >= dealDelay){
        ...
        if (root.current){
          ...
          root.current.rotation.set(0,0,-0.10)
        }
        if (face.current){
          face.current.rotation.y = 0
        }
      } else { return }
    }

    const t = Math.max(0, clock.current - dealDelay)
    const p = Math.min(1, t / totalMs)  // 0 → 1 across the whole anim
    const e = easeInOutQuint(p)

    // rotation: 0 → PI * sign by touchdown
    if (face.current){
      face.current.rotation.y = Math.PI * sign * e
    }

    // position along eased lerp with an arc lift
    if (root.current){
  ...
  return (
    <group ref={root}>
      {/* single face that shows back/front based on orientation */}
      <mesh ref={face} position={[0,0,eps]} renderOrder={2}>
        <planeGeometry args={[w,h]} />
        <CardFace frontTex={frontTex} backTex={backTex} />
      </mesh>
    </group>
  )
}
...
// ---------- Main ----------
export default function CardDealTwo_DeckFlip_v20({
  c1, c2,
  animateKey,
  height=220,
  basePath='/cards/meuk',
  ...
  zLift=0.035,
  cameraZoom=180,
  firstDelayMs=250, staggerMs=900, totalMs=1000,
}){
  const backTex = useTextureCandidates(buildBackCandidates(basePath), [basePath])

  const key1 = `${animateKey}-1-${c1?.label}-${c1?.suit}`
  const key2 = `${animateKey}-2-${c2?.label}-${c2?.suit}`
  ...
  return (
    <div style={{ width:'100%', height, position:'relative', overflow:'visible' }}>
      ...
      >
        ...
        {backTex && <DeckStack backTex={backTex} x={deckX} y={deckY} />}

        {c1 && (
          <FlipThenTravelCard
            key={key1}
            label={c1.label} suit={c1.suit}
            basePath={basePath}
            deck={[deckX, deckY, 0]}
            target={[leftX, 0, 0]}
            finalRotZ={-0.05}
